...
export default class CandleTooltipView extends IndicatorTooltipView {
  override drawImp (ctx: CanvasRenderingContext2D): void {
    ...
    // Always show labels (TradingView style) — not gated on crosshair hover
  ...
  private _drawCandleStandardTooltip (
    ...
    if (this.isDrawTooltip(crosshair, tooltipStyles)) {
      ...
      // TradingView style: build combined row [date, O, H, L, C, ...]
      ...
      if (tooltipTitleStyles.show) {
        ...
        legends.unshift({ title: { text: '', color }, value: { text, color } })
      }
  ...
  private _getCandleTooltipLegends (): TooltipLegend[] {
    ...
    const dataIndex = chartStore.getCrosshair().dataIndex ?? (dataList.length - 1)
