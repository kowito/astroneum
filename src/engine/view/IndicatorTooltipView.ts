...
export default class IndicatorTooltipView extends View<YAxis> {
  ...
  private readonly _rowBounds = new Map<string, { top: number; bottom: number }>()
  private _hoveredIndicatorId: string | null = null
  ...
  private readonly _toggleCollapseEvent = (indicatorId: string) => () => {
    if (this._collapsed.has(indicatorId)) {
      this._collapsed.delete(indicatorId)
    } else {
      this._collapsed.add(indicatorId)
    }
    return true
  }
  ...
  constructor (widget: DrawWidget<DrawPane<YAxis>>) {
    ...
    this.registerEvent('mouseMoveEvent', (event: MouseTouchEvent) => {
      ...
      for (const [id, bounds] of this._rowBounds) {
        if (event.y >= bounds.top && event.y <= bounds.bottom) {
          this._hoveredIndicatorId = id
          break
        }
      }
      return false
  ...
  override drawImp (ctx: CanvasRenderingContext2D): void {
    ...
    // Always show indicator legends (TradingView style)
    if (chartStore.getDataList().length === 0) return
  ...
  protected drawIndicatorTooltip (
    ...
    if (this.isDrawTooltip(chartStore.getCrosshair(), tooltipStyles)) {
      ...
      // TradingView style: each indicator on its own row, values inline within the row
      ...
      indicators.forEach(indicator => {
        ...
        if (nameValid || legendValid) {
          const isCollapsed = this._collapsed.has(indicator.id)
          const isHovered = indicator.id === this._hoveredIndicatorId
          // Only show action icons when hovering this row (TradingView style)
          const features = isHovered
            ? this.classifyTooltipFeatures(featuresStyles)
            : [[], [], []]

          // Collapse chevron — always visible, click to toggle
          prevRowHeight = this.drawChevron(
            ctx, indicator.id, isCollapsed,
            coordinate, prevRowHeight, tooltipTitleStyles
          )
          ...
          // Values (only when expanded)
          if (!isCollapsed && legendValid) {
            prevRowHeight = this.drawStandardTooltipLegends(
          ...
          // Action buttons at end of row — only show on hover (TradingView style)
          if (isHovered) {
            const allFeatures = [...features[0], ...features[1], ...features[2]]
          ...
          // Each indicator advances top → own row
          top = coordinate.y + prevRowHeight
          this._rowBounds.set(indicator.id, { top: rowStartY, bottom: top })
        }
      })
    }
    return top
  ...
  protected drawChevron (
    ...
    this.createFigure({
      ...
      styles: { color, size, family, weight, paddingLeft: 2, paddingTop: 0, paddingRight: 4, paddingBottom: 0 }
    }, {
      mouseDownEvent: this._toggleCollapseEvent(indicatorId)
    })?.draw(ctx)
    coordinate.x += marginLeft + textWidth + 8
  ...
  protected getIndicatorTooltipData (indicator: Indicator): IndicatorTooltipData {
    ...
    const dataIndex = chartStore.getCrosshair().dataIndex ?? Math.max(0, result.length - 1)
    ...
    if (isFunction(indicator.createTooltipDataSource)) {
      ...
      if (tooltipTitleStyles.show) {
        if (customName !== undefined && tooltipTitleStyles.showName) {
          tooltipData.name = customName
        }
        if (customCalcParamsText !== undefined && tooltipTitleStyles.showParams) {
      ...
      if (customLegends !== undefined && indicator.visible) {
