  ...
  * Whether there are forward and backward more flag
  ...
  constructor (chart: Chart, options?: Options) {
    ...
    this._taskScheduler = new TaskScheduler(() => {
      ...
      // Tick-only updates (same bar, same timestamp) still rebuild Y-axis ticks
      // so the viewport auto-scrolls when the price hits the top/bottom edge.
      // YAxis.getAutoSize() rounds to 4 px so label-width fluctuations don't
      // trigger canvas resize (blink) even with buildYAxisTick enabled.
      const needsFull = this._fullLayoutNeeded
  ...
  private _addData (
    ...
    if (success && adjustFlag) {
      // A "tick update" replaces the last bar in-place (same timestamp, no new bar).
      // These arrive up to 60 fps from the TickAnimator.  Using the full layout
      // (y-axis tick rebuild + width measurement) on every tick causes
      // YAxis.getAutoSize() to run, which may return a slightly different pixel
      // width as the price label text changes → canvas resize → visible blink.
      // Light layout skips those steps; the y-axis ticks are updated only when a
      // genuinely new bar is appended or on the initial / backward / forward loads.
      const isTickUpdate = type === 'update' && dataLengthChange === 0
      ...
      if (filterIndicators.length > 0) {
        ...
        if (isTickUpdate) {
          ...
          // and the viewport auto-scrolls when the price hits the top/bottom edge.
          // YAxis.getAutoSize() now rounds to 4 px, so normal 1-3 px label fluctuations
          // no longer trigger a canvas resize (blink) even with buildYAxisTick enabled.
          this._chart.layout({ buildYAxisTick: true, cacheYAxisWidth: true, update: true })
        } else {
          this._chart.layout({
            measureWidth: true,
  ...
  getDataByDataIndex (dataIndex: number): Nullable<CandleData> {
  ...
  dataIndexToTimestamp (dataIndex: number): Nullable<number> {
    ...
    if (isValid(this._period)) {
      ...
      if (isNumber(referenceTimestamp)) {
        const { timespan, multiplier } = this._period
        switch (timespan) {
          case 'second': {
            return referenceTimestamp + multiplier * 1000 * diff
          }
          case 'minute': {
            return referenceTimestamp + multiplier * 60 * 1000 * diff
          }
          case 'hour': {
            return referenceTimestamp + multiplier * 60 * 60 * 1000 * diff
          }
          case 'day': {
            return referenceTimestamp + multiplier * 24 * 60 * 60 * 1000 * diff
          }
          case 'week': {
            return referenceTimestamp + multiplier * 7 * 24 * 60 * 60 * 1000 * diff
          }
          case 'month': {
            ...
            date.setMonth(date.getMonth() + multiplier * diff)
          ...
          case 'year': {
            const date = new Date(referenceTimestamp)
            date.setFullYear(date.getFullYear() + multiplier * diff)
            return date.getTime()
          }
        }
      }
  ...
  timestampToDataIndex (timestamp: number): number {
    ...
    if (isValid(this._period)) {
      ...
      if (isNumber(referenceTimestamp)) {
        ...
        switch (timespan) {
          case 'second': {
            return baseDataIndex + Math.floor((timestamp - referenceTimestamp) / (multiplier * 1000))
          }
          case 'minute': {
            return baseDataIndex + Math.floor((timestamp - referenceTimestamp) / (multiplier * 60 * 1000))
          }
          case 'hour': {
            return baseDataIndex + Math.floor((timestamp - referenceTimestamp) / (multiplier * 60 * 60 * 1000))
          }
          case 'day': {
            return baseDataIndex + Math.floor((timestamp - referenceTimestamp) / (multiplier * 24 * 60 * 60 * 1000))
          }
          case 'week': {
            return baseDataIndex + Math.floor((timestamp - referenceTimestamp) / (multiplier * 7 * 24 * 60 * 60 * 1000))
          }
          case 'month': {
            ...
            return baseDataIndex + Math.floor(((currentYear - referenceYear) * 12 + (currentMonth - referenceMonth)) / multiplier)
          }
          case 'year': {
            const referenceYear = new Date(referenceTimestamp).getFullYear()
            const currentYear = new Date(timestamp).getFullYear()
            return baseDataIndex + Math.floor((currentYear - referenceYear) / multiplier)
          }
        }
      }
    }
